let keys = document.querySelectorAll('.keys');
let spaceKey = document.querySelector('.space_key');
let shift_left = document.querySelector('.shift_left');
let shift_right = document.querySelector('.shift_right');
let caps_lock_key = document.querySelector('.caps_lock_key');
let toggle_circle = document.querySelector('.toggle_circle');
let night_mode = document.querySelector('.night_mode');
let body = document.querySelector('body');
let text_input = document.querySelector('.text');
let change_color = document.querySelector('.change_light_color');
let colors_input = document.querySelector('.colors_input');
let keyboard_lights = document.querySelector('.keyboard_lights');
let keyboard_wrapp = document.querySelector('.keyboard_wrapp');

for(let i = 0; i < keys.length; i++) {
    keys[i].setAttribute('keyname', keys[i].innerText);
    keys[i].setAttribute('lowerCaseName', keys[i].innerText.toLowerCase());
}

window.addEventListener('keydown', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.key == keys[i].getAttribute('keyname' ) || e.key == keys[i].getAttribute('lowerCaseName')) {
            keys[i].classList.add('active')
        }
        if(e.code == 'Space') {
            spaceKey.classList.add('active')
        }
        if(e.code == 'ShiftLeft') {
            shift_right.classList.remove('active')
        }
        if(e.code == 'ShiftRight') {
            shift_left.classList.remove('active')
        }
        if(e.code == 'CapsLock') {
            caps_lock_key.classList.toggle('active');
        }
    }
    if(e.key == 'Backspace'){
        let str = text_input.value;
        text_input.value = str.slice(0,-1);
    }
    else if(e.key == 'CapsLock' ||e.key == 'Shift' || e.key == 'Control' || e.key == 'Meta' ||  e.key == 'Alt' || e.key == 'ControlAltGraph' || e.key == 'Enter'){}
    else if(e.key == 'Tab'){
        text_input.value += "    ";
    }
    else{
        text_input.value += e.key;
    }
})

window.addEventListener('keyup', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.key == keys[i].getAttribute('keyname' ) || e.key == keys[i].getAttribute('lowerCaseName')) {
            keys[i].classList.remove('active')
            keys[i].classList.add('remove')
        }
        if(e.code == 'Space') {
            spaceKey.classList.remove('active');
            spaceKey.classList.add('remove');
        }
        if(e.code == 'ShiftLeft') {
            shift_right.classList.remove('active')
            shift_right.classList.remove('remove')
        }
        if(e.code == 'ShiftRight') {
            shift_left.classList.remove('active')
            shift_left.classList.remove('remove')
        }
        setTimeout(()=> {
            keys[i].classList.remove('remove')
        },200)
    }
})

keyboard_wrapp.addEventListener('mousedown', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.target.innerText == keys[i].getAttribute('keyname' ) || e.target.innerText == keys[i].getAttribute('lowerCaseName')) {
            keys[i].classList.add('active')
        }
        if(e.target.innerText == 'Space') {
            spaceKey.classList.add('active')
        }
        if(e.target.innerText == 'ShiftLeft') {
            shift_right.classList.remove('active')
        }
        if(e.target.innerText == 'ShiftRight') {
            shift_left.classList.remove('active')
        }
        if(e.target.innerText == 'Caps Lock') {
            caps_lock_key.classList.toggle('active');
        }
    }
    if(String(e.target.innerText) == 'Backspace'){
        let str = text_input.value;
        text_input.value = str.slice(0,-1);
    }
    else if(e.target.innerText == 'Caps Lock' || e.target.innerText == 'Shift' || e.target.innerText == 'Ctrl' || e.target.innerText == '' || e.target.innerText == 'Win' ||  e.target.innerText == 'Alt' || e.target.innerText == 'Fn' || e.target.innerText == 'Enter'){}
    else if(e.target.innerText == 'Tab'){
        text_input.value += "    ";
    }
    else if(e.target.innerText == 'Space'){
        text_input.value += " ";
    }
    else{
        console.log(e.target.innerText)
        text_input.value += e.target.innerText;
    }
})

keyboard_wrapp.addEventListener('mouseup', function(e) {
    for(let i = 0; i < keys.length; i++) {
        if(e.target.innerText == keys[i].getAttribute('keyname' ) || e.target.innerText == keys[i].getAttribute('lowerCaseName')) {
            keys[i].classList.remove('active')
            keys[i].classList.add('remove')
        }
        if(e.target.innerText == 'Space') {
            spaceKey.classList.remove('active');
            spaceKey.classList.add('remove');
        }
        if(e.target.innerText == 'ShiftLeft') {
            shift_right.classList.remove('active')
            shift_right.classList.remove('remove')
        }
        if(e.target.innerText == 'ShiftRight') {
            shift_left.classList.remove('active')
            shift_left.classList.remove('remove')
        }
        setTimeout(()=> {
            keys[i].classList.remove('remove')
        },200)
    }
})

night_mode.addEventListener('click',function() {
    toggle_circle.classList.toggle('active');
    body.classList.toggle('active');
    night_mode.classList.toggle('active');
    keyboard_wrapp.classList.toggle('active');
    text_input.classList.toggle('active');
    change_color.classList.toggle('active');
    for(let i = 0; i < keys.length; i++) {
        keys[i].classList.toggle('keys_night')
    }
})

colors_input.addEventListener('input',function() {
    for(let i = 0; i < keys.length; i++) {
        keys[i].style.color = colors_input.value
    }
    keyboard_lights.style.background = colors_input.value;
})